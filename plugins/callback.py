...
async def cb_handler(c, m):
  ...
  if "register" in cb_data:
      Config.registerr.append(m.from_user.id)
      button = [[InlineKeyboardButton("cancel", callback_data="cancel")]]
      markup = InlineKeyboardMarkup(button)
      await m.message.delete()
      await c.send_message(chat_id=m.message.chat.id, text="Send me your account info in this format. IN GAME NAME, IN GAME I'D, ACCOUNT LEVEL, KD RATIO", reply_markup=markup)

  if "cancel" in cb_data:
      if m.from_user.id in Config.registerr:
         Config.registerr.remove(m.from_user.id)
  ...
  if "yes" in cb_data:
      ...
      await m.message.delete()
      await c.send_message(chat_id=m.message.chat.id, text="Message delivered successfully")
